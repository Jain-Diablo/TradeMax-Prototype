...
function App() {
  ...
  const handleStockChange = async (event) => {
    ...
    if (activeTab === 'trade') {
      try {
        const response = await axios.get(`http://localhost:5000/api/stock-data/${newStock}`, {
          withCredentials: true
        });
  ...
  const handleClick = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get(`http://localhost:5000/api/stock-data/${selectedStock}`, {
        withCredentials: true
      });
      setStockData(response.data);
    } catch (error) {
  ...
  const getChartData = (historicalData) => ({
    ...
    datasets: [
      {
        label: 'Stock Price',
        data: historicalData.map(item => parseFloat(item.price)),
        borderColor: '#61dafb',
  ...
  const executeTrade = async (type, symbol, shares) => {
    try {
      const priceResponse = await axios.get(`http://localhost:5000/api/stock-data/${symbol}`, {
        withCredentials: true
      });
      const price = priceResponse.data.price;

      await axios.post(`http://localhost:5000/api/portfolio/${type}`, {
        symbol,
  ...
  const calculateMaxShares = async () => {
    try {
      const priceResponse = await axios.get(`http://localhost:5000/api/stock-data/${selectedStock}`, {
        withCredentials: true
      });
      const price = priceResponse.data.price;
            ...
            {stockData && (
              <>
                <div className="stock-data">
                  ...
                  <p className={parseFloat(stockData.change) >= 0 ? 'positive' : 'negative'}>
                ...
                {stockData.historical && stockData.historical.length > 0 && (
                  <div className="chart-container">
                    <Line options={chartOptions} data={getChartData(stockData.historical)} />
                  </div>
                )}
              </>
            )}
        ...
        return (
          <>
            ...
            <div className="trading-controls">
              <button 
                className="calc-max-btn" 
                onClick={calculateMaxShares}
              >
              ...
              />
              <button onClick={() => executeTrade('buy', selectedStock, shares)}>Buy</button>
              <button onClick={() => executeTrade('sell', selectedStock, shares)}>Sell</button>
            </div>
            {error && <p className="error">{error}</p>}
          </>
        );
